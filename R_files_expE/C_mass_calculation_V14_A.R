###############################################################################################################################################
################### Carbon flux balance calculations - frontend script
###################
# Author: Julian Rogger
# Publication: Speed of Thermal Adaptation of Terrestrial Vegetation alters Earth's long-term climate

# Currently coded to compute about 432 model runs in parallel, in chunks of 18 models per CPU
###############################################################################################################################################

# Define parameter space 
thermal_adapt_par_space <- c(0.01, 0.025, 0.05, 0.1, 0.25, 0.5, 1, 60)/1e+6
dispersal_par_space <- c(450, 550, 650, 750, 850, 4000)
...
pars_list_I <- pars_list[145:162]
pars_list_J <- pars_list[163:180]
pars_list_K <- pars_list[181:198]
pars_list_L <- pars_list[199:216]
pars_list_M <- pars_list[217:234]
pars_list_N <- pars_list[235:252]
pars_list_O <- pars_list[253:270]
pars_list_P <- pars_list[271:288]
pars_list_Q <- pars_list[289:306]
pars_list_R <- pars_list[307:324]
pars_list_S <- pars_list[325:342]
pars_list_T <- pars_list[343:360]
pars_list_U <- pars_list[361:378]
pars_list_V <- pars_list[379:396]
pars_list_W <- pars_list[397:414]
pars_list_X <- pars_list[415:432]


# Intialise cluster
library(parallel)
cl <- makeCluster(18, outfile = "./out_report_A.txt", overwrite = F)
...
out <- parLapply(cl = cl, pars_list_A, fun = function(list_entry){
  ...
  prefix_geochemical_input <- "./geochemical_input_data/"
  ...
  for(t in timesteps){
    ...
    # Go to directory of biological model
    setwd(paste(experiment_directory,"/gen3sis/", sep = ""))
    
    # Landscape depends on CO2 trajectory
    if(CO2_range == "mid"){
      landscape_directory <- "../../model_landscape/landscape_smooth_midCO2"
    } else if(CO2_range == "high"){
      landscape_directory <- "../../model_landscape/landscape_smooth_highCO2"
    } else if(CO2_range == "low"){
      landscape_directory <- "../../model_landscape/landscape_smooth_lowCO2"
    }
    ...
    # Adaptation scaling
    output$temp_adapt_limit <-  1 * exp(-0.25 * (output$Tdiff^2)) # Sensitivity of productivity to adaptation state
    ...
    # Productivity 
    burial_rate <- 0.0007 # assumed burial rate - not relevant as fluxes get calibrated at end of model run again
    ...
    # tot_erosion     # Calibration of k_e --> 1.6e+10 erosion
      
    # Runoff dependency 
    k_w <- 1e-3 # Silicate weathering flow dependence
    ...
    f_E[is.infinite(f_E)] = 2e+7  # Cap excessive values if erosion goes to zero
    ...
    bio_enhancement = ( 1 - pmin(output$weathering_limit_adapt , 1, na.rm=FALSE) ) * PREPLANT + (output$weathering_limit_adapt)
    ...
    # Preliminary calibration for model development
    ...
    oxidw <- k_oxidw * (tot_erosion/1.625522) * ((O2_current/O2_present_day)^a)
...
saveRDS(out, "compiled_output_A.rds")
