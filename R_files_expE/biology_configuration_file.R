...
#
# Publications:Speed of thermal adaptation of the terrestrial vegetation alters Earth's long term climate
# Description: 
######################################
...
create_ancestor_species <- function(landscape, config) {
  ...
  if(timestep == 780){
    ...
    landcells <- length(landscape$environment[, 1])
    all_species <- list()
...
# threshold for genetic distance after which a speciation event takes place. Not relevant for current work.
...
get_divergence_factor <- function(species, cluster_indices, landscape, config) {
  ...
  return(1) # as soon as "species"/"floras" are separated they evolve separately the next timestep
}
...
apply_evolution <- function(species, cluster_indices, landscape, config){
  ...
  # evolve Topt
  temperature_adaptation_change <- landscape$environment[cells, "temp"] - traits[cells, "Topt"]
...
apply_ecology <- function(abundance, traits, landscape, config) {
  ...
  Tair_limit <- 1 * exp(-0.25 * (Tair_diff^2))
