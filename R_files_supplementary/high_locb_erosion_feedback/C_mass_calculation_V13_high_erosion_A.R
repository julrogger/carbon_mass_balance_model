...
# Intialise cluster
library(parallel)
cl <- makeCluster(18, outfile = "./out_report_high_erosion_A.txt", overwrite = F)
...
out <- parLapply(cl = cl, pars_list_A, fun = function(list_entry){
  ...
  for(t in timesteps){
    ...
    # Spatially explicit with evolution 
      ...
      # Scaling terrestrial organic carbon burial with normalized erosion rate
      output$productivity_adapt_sum_global <- output$productivity_adapt_sum_global * (tot_erosion/1.625522) * 4
      ...
      f_E[is.infinite(f_E)] = 2e+7  # This needs to be checked. 
...
saveRDS(out, "compiled_output_high_erosion_A.rds")
