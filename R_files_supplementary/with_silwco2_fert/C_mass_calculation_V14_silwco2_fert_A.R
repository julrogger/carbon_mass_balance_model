...
# Intialise cluster
library(parallel)
cl <- makeCluster(18, outfile = "./out_report_co2_fert_A.txt", overwrite = F)
 ...
 # Prefixes - Euler vs. Local 
  ...
  for(t in timesteps){
    ...
    # Spatially explicit with evolution 
      ...
      # Add GEOCARB CO2 fertilization effect 
      # weathering_limit_adapt can now exceed 1 - should not be a problem in silicate weathering function
      output$productivity_adapt <- output$productivity_adapt * ((2 * (CO2_current/276.01))/(1 + (CO2_current/276.01)))^0.4
      output$productivity_adapt_sum_global <- sum(output$productivity_adapt, na.rm=TRUE)
      output$weathering_limit_adapt <- output$weathering_limit_adapt * ((2 * (CO2_current/276.01))/(1 + (CO2_current/276.01)))^0.4
      output$physiological_limit <- output$physiological_limit * ((2 * (CO2_current/276.01))/(1 + (CO2_current/276.01)))^0.4
      ...
      bio_enhancement = ( 1 - pmin(output$weathering_limit_adapt , 1, na.rm=FALSE) ) * PREPLANT * ((CO2_current/278)^0.5) + (output$weathering_limit_adapt) # * ((CO2_current/278)^0.5)
...
saveRDS(out, "compiled_output_co2_fert_A.rds")
