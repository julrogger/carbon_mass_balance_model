...
# Intialise cluster
library(parallel)
cl <- makeCluster(9, outfile = "./out_report_A.txt", overwrite = F)
...
out <- parLapply(cl = cl, pars_list, fun = function(list_entry){
  ...
  dir.create(paste(experiment_directory, "/gen3sis/model_output/biology_configuration_file_A/", sep = ""), showWarnings = FALSE)
  dir.create(paste(experiment_directory, "/gen3sis/model_output/biology_configuration_file_A/world_trait_map/", sep = ""), showWarnings = FALSE)
  file.copy("./biology_configuration_file_A.R", paste(experiment_directory, "/gen3sis/", sep = ""))
  ...
  for(t in timesteps){
    ...
    # Spatially explicit with evolution 
      ...
      biosphere_simulation <- run_simulation(config = "./biology_configuration_file_A.R",
      ...
      landscape <- readRDS(paste(experiment_directory,"/gen3sis/model_output/biology_configuration_file_A/landscapes/landscape_t_",current_gen3sis_time_step,".rds", sep=""))
      ...
      biome_traits <- readRDS(paste(experiment_directory,"/gen3sis/model_output/biology_configuration_file_A/world_trait_map/map_t_",current_gen3sis_time_step,".rds", sep=""))
