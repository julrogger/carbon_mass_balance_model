...
thermal_adapt_par_space <- c(0.025)/1e+6
dispersal_par_space <- c(650)
...
# Intialise cluster
library(parallel)
cl <- makeCluster(9, outfile = "./out_report_D.txt", overwrite = F)

out <- parLapply(cl = cl, pars_list, fun = function(list_entry){
  ...
  dir.create(paste(experiment_directory, "/gen3sis/model_output/biology_configuration_file_D/", sep = ""), showWarnings = FALSE)
  dir.create(paste(experiment_directory, "/gen3sis/model_output/biology_configuration_file_D/world_trait_map/", sep = ""), showWarnings = FALSE)
  file.copy("./biology_configuration_file_D.R", paste(experiment_directory, "/gen3sis/", sep = ""))
  ...
  for(t in timesteps){
    ...
    # Spatially explicit with evolution 
      ...
      biosphere_simulation <- run_simulation(config = "./biology_configuration_file_D.R",
      ...
      landscape <- readRDS(paste(experiment_directory,"/gen3sis/model_output/biology_configuration_file_D/landscapes/landscape_t_",current_gen3sis_time_step,".rds", sep=""))
      ...
      biome_traits <- readRDS(paste(experiment_directory,"/gen3sis/model_output/biology_configuration_file_D/world_trait_map/map_t_",current_gen3sis_time_step,".rds", sep=""))
      ...
      # Adaptation scaling
      output$temp_adapt_limit <- 1 * exp(-0.02 * (output$Tdiff^2))
...
saveRDS(out, "compiled_output_D.rds")
