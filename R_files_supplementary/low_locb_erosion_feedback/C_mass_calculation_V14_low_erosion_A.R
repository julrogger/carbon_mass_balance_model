...
# Intialise cluster
library(parallel)
cl <- makeCluster(18, outfile = "./out_report_low_erosion_A.txt", overwrite = F)
...
out <- parLapply(cl = cl, pars_list_A, fun = function(list_entry){
  ...
  for(t in timesteps){
    ...
    # Spatially explicit with evolution 
      ...
      output$productivity_adapt_sum_global <- output$productivity_adapt_sum_global * (tot_erosion/1.625522)
...
saveRDS(out, "compiled_output_low_erosion_A.rds")
